// src/components/CompanyDirectory.jsx
'use client';
...
// ---- Static Data ----
const CATEGORIES = ['MNC', 'Startup', 'SME', 'Government', 'NGO', 'PSU', 'Other'];
const INDUSTRIES = [
  'IT & Software', 'Banking & Finance', 'Healthcare', 'Manufacturing',
  'Retail & E-Commerce', 'Telecom', 'Education', 'Logistics', 'Real Estate',
  'Media & Entertainment', 'Consulting', 'Hospitality', 'Automotive', 'Other'
];
const METHODS = ['Email', 'Phone', 'Portal', 'Manual', 'API'];
...
const CompanyDirectory = () => {
  ...
  const fetchCompanies = async () => {
    ...
    try {
      ...
      alert('Failed to load company directory. Please ensure backend is running on port 5000.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (company) => {
    setFormData({
      ...
      method: company.method || 'Email',
      avgDays: company.avgDays ?? '',
      notes: company.notes || '',
  ...
  const clearFilters = () => {
    setFilterCriteria({ companyName: '', industry: '', category: 'all', status: 'all' });
    setSearchQuery('');
  };
  ...
  const filteredCompanies = useMemo(() => {
    return companies.filter(c => {
      ...
      const matchesSearch = !query ||
        ...
        c.alsoKnownAs?.toLowerCase().includes(query) ||
        c.industry?.toLowerCase().includes(query) ||
        c.hrEmail?.toLowerCase().includes(query);

      const matchesName = !filterCriteria.companyName ||
        c.companyName?.toLowerCase().includes(filterCriteria.companyName.toLowerCase());

      const matchesIndustry = !filterCriteria.industry ||
        c.industry?.toLowerCase().includes(filterCriteria.industry.toLowerCase());

      const matchesCategory = filterCriteria.category === 'all' || c.category === filterCriteria.category;

      const matchesStatus = filterCriteria.status === 'all'
        ? true
        : filterCriteria.status === 'active' ? c.isActive : !c.isActive;

      return matchesSearch && matchesName && matchesIndustry && matchesCategory && matchesStatus;
    });
  }, [companies, filterCriteria, searchQuery]);
          ...
          <div className="md:w-1/2">
            <div className="flex items-center gap-4 mb-3">
              <div className="w-12 h-12 rounded-xl bg-[#00D4AA]/10 flex items-center justify-center border border-[#00D4AA]/20">
                <IoBusinessOutline size={24} className="text-[#00A88A]" />
              </div>
              <h2 className="text-4xl font-bold text-[#0A1628] tracking-tight">Company Directory</h2>
            </div>
            <p className="text-[#5C6B7A] text-base leading-relaxed flex items-center gap-2">
              <IoBriefcaseOutline size={18} className="text-[#7A8B9A]" />
              Employer directory for employment verification
            </p>
          </div>
 ...
 ${Object.values(filterCriteria).some(val => val && val !== 'all') || searchQuery
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {[
            { label: 'TOTAL', val: companies.length, color: 'text-[#00A88A]' },
            { label: 'ACTIVE', val: companies.filter(c => c.isActive).length, color: 'text-[#059669]' },
            { label: 'INACTIVE', val: companies.filter(c => !c.isActive).length, color: 'text-[#E53E3E]' },
          ].map((stat, i) => (
        ...
        {/* TABLE / GRID */}
        {loading ? (
          <div className="text-center py-20 text-[#5C6B7A]">Loading company directory...</div>
        ) : (
          <>
            {/* LIST VIEW */}
            {viewType === 'list' && (
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-black/5 bg-black/[0.02]">
                        <th className="p-5 text-xs font-bold text-[#7A8B9A] tracking-wider uppercase">Company</th>
                        <th className="p-5 text-xs font-bold text-[#7A8B9A] tracking-wider uppercase">Category</th>
                        <th className="p-5 text-xs font-bold text-[#7A8B9A] tracking-wider uppercase">Industry</th>
                    ...
                    <tbody>
                      {filteredCompanies.length > 0 ? (
                        filteredCompanies.map((company) => (
                          <tr key={company._id} className="border-b border-black/5 hover:bg-black/[0.02] transition-colors group">
                            <td className="p-5">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <div className="font-medium text-[#0A1628] text-sm">{company.companyName}</div>
                                  {company.alsoKnownAs && (
                                    <div className="text-xs text-[#7A8B9A] mt-0.5">aka {company.alsoKnownAs}</div>
                                  )}
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="p-5 text-sm text-gray-600">{company.industry || '—'}</td>
                            <td className="p-5">
                              <span className="px-3 py-1.5 rounded-lg text-xs font-medium border bg-[#00D4AA]/10 text-[#00A88A] border-[#00D4AA]/20">
                                {company.method || '—'}
                              </span>
                            </td>
                            <td className="p-5 text-sm text-gray-600">{company.avgDays ? `${company.avgDays} days` : '—'}</td>
                            <td className="p-5">
                              <div className="flex items-center gap-2">
                                <div className={`w-2.5 h-2.5 rounded-full ${company.isActive ? 'bg-[#10B981]' : 'bg-gray-400'}`}></div>
                                <span className="text-sm text-gray-600">{company.isActive ? 'Active' : 'Inactive'}</span>
                              </div>
                            </td>
                            <td className="p-5 text-right">
                              <div className="flex items-center justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleView(company)} className="p-2 rounded-lg hover:bg-[#00D4AA]/10 hover:text-[#00A88A] transition-all"><IoViewIcon size={18} /></button>
                                <button onClick={() => handleEdit(company)} className="p-2 rounded-lg hover:bg-[#3B82F6]/10 hover:text-[#3B82F6] transition-all"><IoPencilOutline size={18} /></button>
                                <button onClick={() => handleDeleteClick(company._id)} className="p-2 rounded-lg hover:bg-[#FF5252]/10 hover:text-[#FF5252] transition-all"><IoTrashOutline size={18} /></button>
                              </div>
                            </td>
                          </tr>
                        ))
                      ) : (
                        <tr><td colSpan={7} className="p-16 text-center text-[#5C6B7A]">No companies found.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            {viewType === 'grid' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-fade-in-up">
                {filteredCompanies.length > 0 ? (
                  filteredCompanies.map((company) => (
                    <div key={company._id} className="group relative bg-white border border-black/10 rounded-2xl p-6 hover:border-black/20 hover:shadow-xl transition-all duration-300 flex flex-col h-full shadow-sm">
                      <div className="flex items-start justify-between mb-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h3 className="font-bold text-[#0A1628] text-sm leading-tight">{company.companyName}</h3>
                            {company.alsoKnownAs && <p className="text-[10px] text-[#7A8B9A] mt-0.5">aka {company.alsoKnownAs}</p>}
                          </div>
                        </div>
                        <div className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border uppercase ${company.isActive ? 'border-[#10B981] text-[#059669] bg-[#10B981]/10' : 'border-gray-300 text-gray-500 bg-gray-500/10'}`}>
                          {company.isActive ? 'Active' : 'Inactive'}
                        </div>
                      </div>
                      <div className="space-y-3 mb-6 flex-1">
                        <div className="flex items-center gap-3 text-xs text-gray-600"><IoGridOutline size={14} /> {company.category || '—'}</div>
                        <div className="flex items-center gap-3 text-xs text-gray-600"><IoBriefcaseOutline size={14} /> {company.industry || '—'}</div>
                        <div className="flex items-center gap-3 text-xs text-gray-600"><IoMailOutline size={14} /> {company.hrEmail || '—'}</div>
                        <div className="flex items-center gap-3 text-xs text-gray-600"><IoCheckmarkCircleOutline size={14} /> {company.method || '—'} {company.avgDays ? `· ${company.avgDays}d` : ''}</div>
                      </div>
                      <div className="pt-4 border-t border-black/5 flex justify-between items-center opacity-70 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleView(company)} className="text-xs font-medium flex items-center gap-1 hover:text-[#00A88A]">View Details</button>
                        <div className="flex gap-2">
                          <button onClick={() => handleEdit(company)} className="p-2 rounded-lg bg-black/5 hover:bg-[#3B82F6] hover:text-white transition-all text-[#3B82F6]"><IoPencilOutline size={15} /></button>
                          <button onClick={() => handleDeleteClick(company._id)} className="p-2 rounded-lg bg-black/5 hover:bg-[#FF5252] hover:text-white transition-all text-[#FF5252]"><IoTrashOutline size={15} /></button>
                        </div>
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="col-span-full p-16 text-center text-[#5C6B7A]">No companies found.</div>
                )}
              </div>
            )}
          </>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-3xl bg-white border border-black/10 rounded-3xl shadow-2xl overflow-hidden animate-fade-in-up max-h-[90vh] flex flex-col">
              <div className="px-8 py-6 border-b border-black/5 flex justify-between items-center bg-black/[0.02]">
                <div>
                  <h3 className="text-2xl font-bold text-[#0A1628]">{editingId ? 'Edit Company' : 'New Company Directory'}</h3>
                  <p className="text-xs text-[#5C6B7A] uppercase tracking-widest mt-1">{editingId ? 'Update Company Details' : 'Add Employer for Verification'}</p>
                </div>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-8 overflow-y-auto custom-scrollbar flex-1">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    />
                    <div>
                      <label className="block text-xs font-medium text-[#5C6B7A] mb-2">CATEGORY</label>
                      <select
                        name="category"
                        value={formData.category}
                        onChange={handleInputChange}
                        className="w-full bg-white border border-black/10 rounded-xl px-4 py-3 outline-none focus:border-[#00D4AA] text-[#0A1628]"
                      >
                    ...
                    />
                    <div>
                      <label className="block text-xs font-medium text-[#5C6B7A] mb-2">INDUSTRY</label>
                      <select
                        name="industry"
                ...
                {/* HR CONTACT */}
                <div className="space-y-6">
                  <SectionHeader title="HR Contact" color="#3B82F6" />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FloatingInput
                      label="HR Email"
                      name="hrEmail"
                ...
                <div className="space-y-6">
                  <SectionHeader title="Verification Details" color="#D9820F" />

                  <div>
                    <label className="block text-xs font-medium text-[#5C6B7A] mb-3">
                      METHOD <span className="text-[#FF5252]">*</span>
                    </label>
 ...
 ${formData.method === m
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={handleInputChange}
                      rows={3}
                      placeholder="Enter any notes about this employer..."
                ...
                <div className="flex gap-4 pt-6 border-t border-black/5">
                  ...
                  >
                    ...
                    ) : editingId ? 'Update Company' : 'Create'}
                  </button>
                </div>
              </form>
            </div>
        ...
        {isDeleteModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-white border border-red-500/20 rounded-2xl p-8 shadow-2xl animate-fade-in-up text-center">
              ...
              <h3 className="text-xl font-bold text-[#0A1628] mb-2">Delete Company?</h3>
            ...
            <div className="relative w-full max-w-lg bg-white border border-black/10 rounded-2xl p-0 shadow-2xl animate-fade-in-up overflow-hidden max-h-[90vh] flex flex-col">
              ...
              <div className="p-8 space-y-5 overflow-y-auto custom-scrollbar">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-2xl font-bold text-[#0A1628]">{viewingCompany.companyName}</h3>
                    {viewingCompany.alsoKnownAs && (
                      <p className="text-sm text-[#5C6B7A] mt-1">aka {viewingCompany.alsoKnownAs}</p>
                    )}
                  </div>
                  <span className={`px-4 py-1.5 rounded-full text-xs font-bold border ${viewingCompany.isActive ? 'border-[#10B981] text-[#059669] bg-[#10B981]/10' : 'border-gray-300 text-gray-500 bg-gray-500/10'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  {[
                    ...
                    ['HR Email', viewingCompany.hrEmail],
                    ['HR Phone', viewingCompany.hrPhone],
                    ['Method', viewingCompany.method],
                    ['Avg Days', viewingCompany.avgDays ? `${viewingCompany.avgDays} days` : null],
                  ].map(([label, val], i) => (
                  ...
                  {viewingCompany.notes && (
                    <div className="col-span-2 p-4 bg-black/5 rounded-xl">
                      ...
                      <div className="text-[#0A1628] text-sm">{viewingCompany.notes}</div>
                    </div>
                  )}
                </div>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-white border border-black/10 rounded-2xl shadow-2xl animate-fade-in-up p-8">
              ...
              <div className="space-y-5">
                <div>
                  <label className="block text-xs font-medium text-[#5C6B7A] mb-2">Company Name</label>
                  <input type="text" name="companyName" value={filterCriteria.companyName} onChange={handleFilterChange} className="w-full bg-white border border-black/10 rounded-xl px-4 py-3 outline-none text-[#0A1628]" placeholder="e.g. Infosys" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-[#5C6B7A] mb-2">Industry</label>
                  <input type="text" name="industry" value={filterCriteria.industry} onChange={handleFilterChange} className="w-full bg-white border border-black/10 rounded-xl px-4 py-3 outline-none text-[#0A1628]" placeholder="e.g. IT & Software" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-[#5C6B7A] mb-2">Category</label>
                  <select name="category" value={filterCriteria.category} onChange={handleFilterChange} className="w-full bg-white border border-black/10 rounded-xl px-4 py-3 outline-none text-[#0A1628]">
